...
public class MainPageViewCommandTest {
    @InjectMocks
    private MainPageViewCommand mainPageView;
    ...
    public void shouldReturnMainPageWithMovies() {
        String expectedPath = "pages/homepage.jsp";
        when(movieFacade.getAllMovies(true)).thenReturn(movieDtosList);

        String resultPath = mainPageView.execute(request);
        verify(movieFacade).getAllMovies(true);
    ...
    public void shouldReturnTruePermission() {
        assertTrue("permission is false", mainPageView.checkPermissions(request));
    }
}
